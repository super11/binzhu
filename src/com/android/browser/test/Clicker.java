...
class Clicker {
	...
	public void clickOnWebElement(String textName, int match, boolean scroll) {
		...
		WebElement webElementToClick = waiter.waitForWebElement(textName, match, TIMEOUT, scroll);

		if(webElementToClick == null){
			if(match > 1)
				Assert.assertTrue(match + " web elements with " + ": '" + "' are not found!", false);
			else
				Assert.assertTrue("Web element with "  +  "is not found", false);
