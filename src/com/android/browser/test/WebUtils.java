...
class WebUtils {
	...
	private boolean executeJavaScript(final By by){
		...
		else if(by instanceof By.CheckBox){
			return executeJavaScriptFunction("CheckBox(\""+by.getValue()+"\");");
		}
		else if(by instanceof By.GetFrameItem){
			return executeJavaScriptFunction("GetFrameItem(\""+by.getValue()+"\",\""+by.getTagName()+"\");");
		}
		else if(by instanceof By.GetFrameSrc){
			return executeJavaScriptFunction("getFrameSrc(\""+by.getValue()+"\");");
		}
		return false;
	}
