...
class Waiter {
   ...
   //add by zhubin
	public WebElement waitForWebElement(String textName, int match,int timeout, boolean scroll) {
		...
		while (true) {	
			...
			if (timedOut){
				searcher.logMatchesFound(textName);
				return null;
			}
			sleeper.sleep();

			WebElement webElementToReturn = searcher.searchForWebElement(textName, match, timeout, scroll); 
